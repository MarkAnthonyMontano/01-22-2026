...
app.use(cors({
  origin: ["http://localhost:5173", "http://192.168.1.10:5173"],  // ✅ Explicitly allow Vite dev server
...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    // QR Codes
    const qrData = `192.168.1.10:5173/examination_profile/${applicant_number}`;
    const qrData2 = `192.168.1.10:5173/applicant_profile/${applicant_number}`;
...
app.post("/login_applicant", async (req, res) => {
  ...
  try {
    ...
    if (existing.length === 0) {
      ...
      // Generate QR code
      const qrData = `192.168.1.10:5173/examination_profile/${applicantNumber}`;
...
app.post("/program_tagging", async (req, res) => {
  ...
  try {
    // Prevent duplicate program tagging (fees NOT part of uniqueness)
    const [existing] = await db3.query(
      `
    ...
    // Insert program tag with fees
    const insertQuery = `
      INSERT INTO program_tagging_table
      (curriculum_id, year_level_id, semester_id, course_id, lec_fee, lab_fee)
      VALUES (?, ?, ?, ?, ?, ?)
    `;
...
app.put("/program_tagging/:id", async (req, res) => {
  ...
  try {
    // Prevent duplicate when updating
    const [existing] = await db3.query(
      `
      SELECT program_tagging_id
      FROM program_tagging_table
    ...
    // Update program tag including fees
    const updateQuery = `
    ...
    res.status(500).json({
      error: "Failed to update program tag",
      details: err.message
    });
  }
});
...
app.get("/api/programs/availability/:yearId/:semesterId", async (req, res) => {
  ...
  try {
    ...
    const [programs] = await db3.query(`
      SELECT
        d.dprtmnt_id,
        d.dprtmnt_name,
        d.dprtmnt_code,
        ...
        p.major
      FROM dprtmnt_table d
      JOIN dprtmnt_curriculum_table dc ON dc.dprtmnt_id = d.dprtmnt_id
      JOIN program_table p ON p.program_id = dc.curriculum_id
      ORDER BY d.dprtmnt_name, p.program_description
    `);
    ...
    const merged = programs.map(p => {
      const slot = slots.find(s => s.program_id === p.program_id);
      const max_slots = slot?.max_slots || 0;
      const total_applicants = slot?.total_applicants || 0;
      return {
        ...p,
        max_slots,
        total_applicants,
        remaining: max_slots - total_applicants
      };
    });
...
app.post("/api/apply-program", async (req, res) => {
  const { person_id, program_id, year_id, semester_id } = req.body;
  ...
  try {
    ...
    const [[slot]] = await conn.query(
      `SELECT max_slots
       ...
       FOR UPDATE`,
      [program_id, year_id, semester_id]
    );
    ...
    const [existing] = await conn.query(
      `SELECT applied_id
       FROM applied_programs
       WHERE person_id = ? AND year_id = ? AND semester_id = ?`,
      [person_id, year_id, semester_id]
    );
    ...
    // ✅ Consume slot
    await conn.query(
      `INSERT INTO applied_programs (person_id, program_id, year_id, semester_id)
       VALUES (?, ?, ?, ?)`,
      [person_id, program_id, year_id, semester_id]
    );
