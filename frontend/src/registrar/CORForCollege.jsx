...
const CertificateOfRegistration = forwardRef(({ student_number }, divToPrintRef) => {
  ...
  useEffect(() => {
    ...
    const fetchStudent = async () => {
      try {
        ...
        const response = await axios.post(`${API_BASE_URL}/student-tagging`, { studentNumber: student_number }, {
        ...
        const corResponse = await axios.get(`${API_BASE_URL}/student-data/${studentNum}`);
  ...
  const handleSaveToUnifast = async () => {
    try {
      const res = await axios.post(`${API_BASE_URL}/save_to_unifast`, requestedData);
      if (res.data.success) {
  ...
  const handleSaveToMatriculation = async () => {
    try {
      const res = await axios.post(`${API_BASE_URL}/save_to_matriculation`, requestedData);
      if (res.data.success) {
        setSnack({ open: true, message: res.data.message, severity: "success" });
      } else {
        setSnack({ open: true, message: res.data.message || "Failed to save data", severity: "error" });
